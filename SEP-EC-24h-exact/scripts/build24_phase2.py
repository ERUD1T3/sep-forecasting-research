...
def main():
    ...
    for ev in sorted(ship_txt):
        ...
        if prev_end is not None:
            pre = pre[pre['Target Timestamp'] > prev_end]
        pre = keep_prefix_by_coverage(pre, backwards=True)
        ...
        if len(post):
            bg = float(np.median(pd.to_numeric(pre['p16.4_tplus6']))) if len(pre) else \
                 float(pd.to_numeric(r['p16.4_tplus6']).iloc[0])
            pi = pd.to_numeric(post['p16.4_tplus6']).values
            runmin = np.minimum.accumulate(np.maximum(pi, 1e-12))
            over = pi / runmin
            policed = pi > max(bg, 1e-12) * REBOUND_FLOOR_MULT
            hit = np.where((over > REBOUND_MAX) & policed)[0]
            if len(hit):
                post = post.iloc[:hit[0]]
        post = keep_prefix_by_coverage(post)
